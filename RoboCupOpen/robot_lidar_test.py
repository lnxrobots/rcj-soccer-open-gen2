...
class Robot(SoccerRobot):
    ...
    def on_update(self) -> None:
        ...
        target = Vector2(1700 / 2, 2030 / 2 - 100)

        angle = (target - Vector2(self.position.x, 2030 - self.position.y)).get_angle()
        ...
        if self.kicking:
            ...
            self.motor_values = mathf.direction_to_motors(0, spin, 170)

            if (self.see_goal and (self.goal_center_angle < 5 or self.goal_center_angle > 355)) or (not self.see_goal and (self.heading < 20 or self.heading > 340)):
        ...
        elif self.camera_module == FrontCameraModule and self.see_ball:
            self.shoot = False
            if not self.has_ball_in(self.ball_position[0], self.ball_position[1]):
                ...
                if abs(self.position.x - target.x) > 120 or abs(self.position.y - target.y) > 120:
                    spin = mathf.direction_to_spin(-self.heading) * con.SPIN_BALL_SPEED * 0.7
                    self.motor_values = mathf.direction_to_motors(math.degrees(angle), spin, 150)
                else:
                    self.kicking = True
                    self.motor_values = (0, 0, 0, 0)
                    time.sleep(0.5)
